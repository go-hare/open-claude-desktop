...
const LOCAL_AGENT_METHODS = [
  "abandonBridgeEnvironment","addFolderToSession","addTrustedFolder","archive","authorizeDirectMcpServer","delete","deleteBridgeAgentMemory","deleteBridgeSession","deleteLocalSkill","disconnectDirectMcpServer","getAll","getBridgeConsent","getDirectMcpServerStatuses","getGitDiff","getGitDiffStats","getGitInfo","getLocalSkillFiles","getSession","getSessionsBridgeEnabled","getSessionsForScheduledTask","getShellPtyBuffer","getSupportedCommands","getTranscript","getTranscriptFeedback","getTrustedFolders","getWorkingTreeStatus","interactiveAuth_$store$_getState","isFolderTrusted","kickBridgePoll","listLocalSkills","mcpCallTool","mcpListResources","mcpReadResource","noteCuWindowMentions","openOutputsDir","removeTrustedFolder","replaceEnabledMcpTools","replaceRemoteMcpServers","requestFolderTccAccess","resetBridge","resetBridgeSession","respondBridgePermissionPreflight","respondDirectoryServers","respondPluginSearch","respondSlashMenuSkills","respondToToolPermission","resizeShellPty","revealLocalSkill","revokeInteractiveAuth","rewind","saveLocalSkill","searchSessions","sendMessage","sessionsBridgeStatus_$store$_getState","setChromePermissionMode","setDraftSessionFolders","setFocusedSession","setLocalSkillEnabled","setMcpServers","setModel","setPermissionMode","setSessionsBridgeEnabled","shareSession","start","startShellPty","stop","stopShellPty","submitTranscriptFeedback","syncSkills","triggerInteractiveAuth","updateSession","writeShellPty",
] as const;
...
async function getCodeStats(cwd: string | null) {
...
function createSessionHandlers(store: LocalSessionStore, context: IpcHandlerContext, allMethods: readonly string[], bridgeInterface: "LocalSessions" | "LocalAgentModeSessions"): InterfaceHandlers {
  ...
  const dispatchSessionEvent = (type: string, sessionId?: string, session?: unknown) => {
    dispatchBridgeEvent(context.windows.mainView.webContents, "claude.web", bridgeInterface, "onEvent", { type, sessionId, session });
  };
  ...
  const appendPtyData = (sessionId: string, data: Buffer) => {
    ...
    dispatchBridgeEvent(context.windows.mainView.webContents, "claude.web", bridgeInterface, "onEvent", { type: "pty-data", sessionId, data: data.toString("utf8") });
  };
  ...
  const startShell = (sessionId: string, cols?: number, rows?: number) => {
    ...
    if (nodePty) {
      const terminal = nodePty.spawn(shellPath, ["-l"], {
        name: "xterm-256color",
        cols: cols ?? 80,
        rows: rows ?? 24,
        cwd,
      ...
      terminal.onExit(({ exitCode, signal }) => {
        dispatchBridgeEvent(context.windows.mainView.webContents, "claude.web", bridgeInterface, "onEvent", { type: "pty-exit", sessionId, code: exitCode, signal });
        ptys.delete(sessionId);
      });
      return true;
    }
    const child = spawn(shellPath, ["-l"], { cwd, env: { ...process.env, COLUMNS: String(cols ?? 80), LINES: String(rows ?? 24) } });
    const terminal = {
    ...
    child.on("exit", (code, signal) => {
      dispatchBridgeEvent(context.windows.mainView.webContents, "claude.web", bridgeInterface, "onEvent", { type: "pty-exit", sessionId, code, signal });
      ptys.delete(sessionId);
    });
    return true;
  };
  ...
  const realHandlers: InterfaceHandlers = {
    ...
    getSession: async (_event, id) => (asString(id) ? store.getSession(asString(id)!) : null),
    getTranscript: async (_event, id) => (asString(id) ? store.getTranscript(asString(id)!) : []),
    start: async (_event, input) => {
      const session = store.start(asObject(input) as never);
    ...
    forkSession: async (_event, id) => {
      const existing = asString(id) ? store.getSession(asString(id)!) : null;
      const session = store.start({ title: existing ? `${existing.title} fork` : "Forked session", cwd: existing?.cwd, prompt: existing?.messages.at(-1)?.text });
      dispatchSessionEvent("start", session.id, session);
      return session;
    },
    ...
    getSessionsForScheduledTask: async () => store.getAll(),
    getSupportedCommands: async () => ["start", "sendMessage", "stop", "readFileAtCwd", "writeSessionFile", "getGitInfo"],
    ...
    getContextUsage: async (_event, id) => ({ messages: asString(id) ? store.getTranscript(asString(id)!).length : 0 }),
    getCodeStats: async (_event, cwdOrSession) => getCodeStats(cwdFromSession(store, cwdOrSession)),
    getDefaultEffort: async () => "medium",
    getEffort: async (_event, id) => (asString(id) ? store.getSession(asString(id)!)?.effort ?? "medium" : "medium"),
    setEffort: async (_event, id, effort) => (asString(id) ? store.update(asString(id)!, { effort: String(effort ?? "medium") }) : null),
    ...
    setPermissionMode: async (_event, id, mode) => (asString(id) ? store.update(asString(id)!, { permissionMode: String(mode ?? "default") }) : null),
    setModel: async (_event, id, model) => (asString(id) ? store.update(asString(id)!, { model: String(model ?? "") }) : null),
    setVisibility: async (_event, id, visibility) => (asString(id) ? store.update(asString(id)!, { visibility: String(visibility ?? "") }) : null),
    setFocusedSession: async () => true,
    setFastMode: async () => true,
    setAutoFixEnabled: async () => true,
    setMcpServers: async () => true,
    replaceEnabledMcpTools: async () => true,
    replaceRemoteMcpServers: async () => true,
    ...
    respondToToolPermission: async () => true,
    ...
    submitTranscriptFeedback: async () => ({ ok: true }),
    ...
    summarizeSession: async (_event, id) => {
      ...
      return transcript.map((message) => message.text).join("\n").slice(0, 1000);
    },
    summarizeTranscript: async (_event, transcript) => Array.isArray(transcript) ? transcript.map((item) => asObject(item).text ?? "").join("\n").slice(0, 1000) : "",
    getPlanForSession: async () => null,
    popBackgroundTaskSuggestion: async () => null,
    getAgents: async () => [],
    ...
    getLocalSkillFiles: async () => [],
    listLocalSkills: async () => [],
    syncSkills: async () => [],
    deleteLocalSkill: async () => true,
    saveLocalSkill: async () => true,
    revealLocalSkill: async () => true,
    setLocalSkillEnabled: async () => true,
    ...
    mcpReadResource: async () => null,
    mcpCallTool: async (_event, serverName, toolName, input) => ({ ok: false, reason: "mcp_tool_runtime_absent", serverName, toolName, input }),
    ...
    readSessionImageAsDataUrl: async (_event, id, relative) => {
      ...
      const ext = path.extname(target).slice(1) || "png";
    ...
    generateLocalPrContent: async () => null,
    ...
    getGitDiff: async (_event, cwdOrSession, args) => runGitInRepository(cwdFromSession(store, cwdOrSession), ["diff", ...(Array.isArray(args) ? args.map(String) : [])]),
    getGitDiffStats: async (_event, cwdOrSession) => runGitInRepository(cwdFromSession(store, cwdOrSession), ["diff", "--stat"]),
    getDiffFileContent: async (_event, cwdOrSession, filePath) => runGit(cwdFromSession(store, cwdOrSession), ["show", `HEAD:${String(filePath ?? "")}`]),
    ...
    setTrustedSSHHosts: async () => true,
    getSSHGitInfo: async () => null,
    getSSHSupportedCommands: async () => ["git status", "git diff", "git log"],
    resolveSSHSettings: async () => ({}),
    ...
    reviewDiff: async () => null,
    launchUltrareview: async (_event, cwdOrSession) => {
      const cwd = cwdFromSession(store, cwdOrSession);
      if (cwd) await shell.openPath(cwd);
      return true;
    },
    runBashCommand: async (_event, sessionId, command) => runProcess(cwdFromSession(store, sessionId), process.env.SHELL || "/bin/zsh", ["-lc", String(command ?? "")], 60000),
    ...
    startSideChat: async () => null,
    stopSideChat: async () => true,
    ...
    rewind: async () => true,
    teleportToCloud: async (_event, id) => ({ success: false, error: `cloud_teleport_absent:${String(id ?? "")}` }),
    getTeleportReadiness: async () => ({ ready: false, reason: "cloud_teleport_absent" }),
    getDetectedProjects: async () => [],
    getInstalledEditors: async () => ({ vscode: await commandExists("code") }),
    isVSCodeInstalled: async () => commandExists("code"),
    openInVSCode: async (_event, target) => {
      const filePath = asString(target);
      if (!filePath) return false;
      const error = await shell.openExternal(`vscode://file/${encodeURIComponent(filePath)}`);
      return error;
    },
    openInEditor: async (_event, target) => {
      const filePath = asString(target);
      if (!filePath) return false;
